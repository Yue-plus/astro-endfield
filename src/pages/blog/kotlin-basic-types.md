...
`Float` 反映了 IEEE 754 _单精度_，而 `Double` 提供了*双精度*。
...
布尔用 `Boolean` 类型表示，它有两个值：_true_{: .keyword } 与 _false_{: .keyword }。
...
_原始字符串_ 使用三个引号（`"""`）分界符括起来，内部没有转义并且可以包含换行以及任何其他字符:
